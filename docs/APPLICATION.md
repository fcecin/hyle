...
`hyle_services` (module `kv`) is the canonical layer-2 application. The plain KV store (`include/hyle/services/kv/kv_state_machine.h`) treats a payload as an ops batch and snapshots the sorted key/value serialization. The economy (`include/hyle/services/kv/ledger.h`) uses `ctx.proposer` to self-mint the block reward and adds transfers, lazy rent, opportunistic eviction, names, and a native on-chain PoW mint. A mock (`tests/mock_state_machine.h`) implements the seam trivially to test consensus in isolation.
