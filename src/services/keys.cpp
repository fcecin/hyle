...
void save_key(const std::string& path, const KeyPair& kp) {
  ...
  if (fd < 0) throw std::runtime_error("morphe: cannot write key file " + path);
  ::fchmod(fd, 0600);
  ...
  if (wrote != static_cast<ssize_t>(line.size()) || closed != 0)
    throw std::runtime_error("morphe: key file write failed (disk full?): " + path);
}

KeyPair load_key(const std::string& path) {
  std::ifstream f(path);
  if (!f) throw std::runtime_error("morphe: cannot read key file " + path);
  std::string line;
  std::getline(f, line);
