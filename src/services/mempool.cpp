...
Admit Mempool::admit_transfer(const TransferOp& op, uint64_t committed_seq,
  ...
  if (op.to.empty()) return Admit::BadShape;
  if (op.to[0] == ACCOUNT_PREFIX) { if (op.to.size() != 33) return Admit::BadShape; }
  else if (op.to[0] == ENTRY_PREFIX) { if (op.to.size() < 2) return Admit::BadShape; }
  else return Admit::BadShape;
